...
private:                                            //Закрытая зона объявлений
    int years, days, hours, minutes, seconds;       //Переменные для конструктора, с которыми пользователь взаимодейстровать не может

public:                                             //Открытая зона объявлений
    ///Конструктор - набор значений времени y-год d-день h-час m-минуты s-секунды
    Time(int y = 0, int d = 0, int h = 0, int m = 0, int s = 0) : years(y), days(d), hours(h), minutes(m), seconds(s) {}
